...
class B_ColorAdapter1(var context: Context, var colors: Array<String>) :
    RecyclerView.Adapter<B_ColorAdapter1.ViewHolder>() {
    private var selectedPosition = -1

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): B_ColorAdapter1.ViewHolder {
        var v = LayoutInflater.from(parent.context).inflate(R.layout.custom_colors, parent, false)

        return ViewHolder(v)
    }

    override fun onBindViewHolder(holder: B_ColorAdapter1.ViewHolder, position: Int) {
...
//        }
        holder.itemView.colortv.setOnCheckedChangeListener(CompoundButton.OnCheckedChangeListener { buttonView: CompoundButton?, isChecked: Boolean ->
            if (isChecked) {
                B_ColorPicker1.selectcolorIf?.getID(colors.get(position), "1");

            } else {
                B_ColorPicker1.selectcolorIf?.getID(colors.get(position), "0");
